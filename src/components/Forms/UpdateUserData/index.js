...
export const UpdateUserData = React.memo(({ email, isAddClient, cursos, setCursos, setEmail, data, setData, mutation, onClose,setPermissions,permissions }) => {
  const URL = 'link-url';
  ...
  function handleSelect(value,onClose) {
    ...
    const newData = {};
    newData[`${indexAdded}--cpf`] = value?.cpf ? keepOnlyNumbers(value?.cpf) : ''
    newData[`${indexAdded}--name`] = value?.name ? value?.name : ''

    const newPermissions = {};
    value?.permissions && value.permissions.map((permission) => {
    ...
    value?.cursos && value.cursos.map((curso) => {
      ...
      if (curso?.epi) {
        curso.epi.map((epi) => {
          if (!newCursos[`${indexAdded}--${curso.id}--epi`]) newCursos[`${indexAdded}--${curso.id}--epi`] = []
          newCursos[`${indexAdded}--${curso.id}--epi`].push({...epi,lock:isStarted?true:false});
        })
      }
    ...
    <InputsContainer>
      ...
      >
        <InputSearch
          ...
          options={users?users.filter(i=>i?.name&&i?.email&&!emails.includes(i.email) ):[]}
        />
              ...
              <InputUnform
                ...
                startComponent={()=>
                  <BootstrapTooltip title={`Remover membro.`} styletooltip={{transform: 'translateY(5px)'}}>
                    <IconButton style={{margin:-5,marginRight:-10,width:20,zIndex:20,height:20}} onClick={() => handleDeleteMember(index)}  aria-label={'delete'}>
