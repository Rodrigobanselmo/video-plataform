...
const ClassesView = styled.div`
  ...
  > p:nth-child(3) {
    ...
    > span {
      ...
      background-color: ${({ theme, epi }) =>
        epi ? theme.palette.status.success : theme.palette.status.infoD};
      font-weight: bold;
      padding: 1px 5px;
      border-radius: 4px;
      &.green {
...
export const ModuleData = ({
  ...
  const onExcelTestJson = (dataClass) => {
    ...
    return dataClass.questions?.map((question, index) => {
      ...
      if (index === 0) {
        ...
        data.subName =
          subCursos.length > 0
            ? subCursos.find((i) => i.id.includes(dataClass.id))
              ? subCursos.find((i) => i.id.includes(dataClass.id)).name
              : ''
            : '';
  ...
  const handleDownloadFile = async () => {
    ...
    modules.map((module, index) => {
      module.classes.map((cls, indexCLS) => {
        if (cls?.type === 'test') {
          ...
          return modulesClassesArray.push({
            ...testData[0],
            moduleId: module.id,
          });
        }
        ...
        const data = {
          ...
          subName:
            subCursos.length > 0
              ? subCursos.find((i) => i.id.includes(cls.id))
                ? subCursos.find((i) => i.id.includes(cls.id)).name
                : ''
              : '',
    ...
    const schema = [
      schemaData,
      ...modulesTestArray.map(() => schemaTest),
      schemaCombos,
    ];

    const sheets = [
      'Sheet 1',
      ...modulesTestArray.map(
        (i, idx) => 'teste ' + idx + ' - ' + i[0].text.substr(0, 10),
      ),
      'combos',
    ];
  ...
  const resolveExcelFile = async (file, sheet) => {
    ...
    if (!mdData)
      throw 'Seu arquivo excel encontra-se com paginas com formatação fora do esperado';
      ...
      return {
        ...
        answer:
          ...
          Object.values(i.question.options).find(
            (fi) => i.question?.answer && fi === i.question.answer,
          ),
  ...
  const onInputFileChange = async (e) => {
    ...
    try {
      if (e.target.files[0]) {
        ...
        const sheets = await readXlsxFile(e.target.files[0], {
          getSheets: true,
        });
        ...
        if (sheets.length > 1) {
          await Promise.all(
            sheets.map(async (i, idx) => {
              if (i.name === 'combos') {
                const { rows: combs, errors } = await readXlsxFile(file, {
                  schema: schemaCombos,
                  sheet: 'combos',
                });
                setCombos(combs);
        ...
        rows.map((i) => {
          ...
          if (epi) {
            ...
            if (!i?.subGroup)
              throw 'O campo "Nome do pacote caso for vendido separadamente" não pode ser nulo para resposatas "SIM"';
      ...
      {modules.map((module, index) => {
        return (
          <ModuleView key={module.id}>
            <p>
              Módulo {index + 1}: <span>{module.name}</span>
            </p>
              ...
              return (
                <ClassesView epi={cls?.epi} key={cls.id}>
                  {cls?.type == 'test' ? (
                    <>
                      <p>
                        Teste {index + 1}: <span>{cls.name}</span>
                      </p>
                      <p>
                        Número de questões: <span>{cls.numQuestions}</span>
                      </p>
                      <p>
                        Porcentagem para passar:{' '}
                        <span className="green">{percentage}</span>
                      </p>
                    </>
                  ) : (
                    <>
                      <p>
                        Aula {index + 1}: <span>{cls.name}</span>
                      </p>
                      <p>
                        Video URL:{' '}
                        <a target="_blank" href={cls.video}>
                          {cls.video}
                        </a>
                      </p>
                      <p>
                        Vendido separadamente:{' '}
                        <span>{cls?.epi ? 'SIM' : 'NÃO'}</span>
                      </p>
                    </>
                  )}
                </ClassesView>
