...
export function VideoPlayer({ curso }) {
  ...
  const queryModules = queryClient.getQueryData([
    'student',
    cursoId,
    currentUser.uid,
  ]);
  const modules =
    queryModules && queryModules?.student ? queryModules.student[0] : {}; //student
  ...
  const moduleIndex = moduleId
    ? curso.modules.findIndex((i) => i.id == moduleId)
    : 0;
  const classIndex = classId
    ? curso.modules[moduleIndex !== -1 ? moduleIndex : 0].classes.findIndex(
        (i) => i.id == classId,
      )
    : 0;
  const actualClass =
    curso.modules[moduleIndex !== -1 ? moduleIndex : 0].classes[
      classIndex !== -1 ? classIndex : 0
    ];
  ...
  const isLastClass =
    curso.modules[moduleIndex] &&
    curso.modules[moduleIndex].classes &&
    classIndex == curso.modules[moduleIndex].classes.length - 1;

  const nextModuleIndex = isLastClass ? moduleIndex + 1 : moduleIndex;

  const nextClassIndex = isLastClass ? 0 : classIndex + 1;

  const nextModule = curso.modules[nextModuleIndex];
  const nextClass = nextModule
    ? nextModule.classes[nextClassIndex]
    : 'lastModule';

  useEffect(() => {
    console.log('useEffect [moduleId, classId]');
    ...
    if (!ready) {
      if (progress[`${cursoId}//${moduleId}//${classId}`] && percentage < 100) {
        ...
        progressTimeRef.current.style.transform = `translateX(${
          setProgress.percentage * progressRef.current.offsetWidth
        }px)`;
        ...
        progressLoadRef.current.style.width = `0%`;
        progressRef.current.value = `1`;
        // }, 1000);
        progressTimeRef.current.style.transform = `translateX(1px)`;
        progressTimeRef.current.innerText = format(
  ...
  const handleProgress = (changeState) => {
    ...
    const pgr = refProgress.current[`${cursoId}//${moduleId}//${classId}`]
      ? refProgress.current[`${cursoId}//${moduleId}//${classId}`].seconds
      : 0;
    console.log('pgr', pgr, changeState);
    ...
    console.log('ready');
    if (
      Math.floor(changeState.playedSeconds) % 5 == 0 &&
      save == 0 &&
      Math.floor(changeState.playedSeconds) > pgr &&
      percentage < 100
    ) {
      console.log('SAVED');
      save = 1;
    ...
    if (!state.seeking && ready) {
      ...
      progressTimeRef.current.style.transform = `translateX(${
        changeState.played * progressRef.current.offsetWidth
      }px)`;
      progressRef.current.value = `${changeState.played * 100 * 4}`;
      progressLoadRef.current.style.width = `${changeState.loaded * 100}%`;

      const translate = progressMark.current.style.transform
        .replace('translateX(', '')
        .replace('px)', '');
      if (
        translate <= changeState.played * progressRef.current.offsetWidth ||
        !translate
      ) {
        progressMark.current.style.transform = `translateX(${
          changeState.played * progressRef.current.offsetWidth
  ...
  const handleSeekChange = (e, newValue) => {
    ...
    if (progressMark.current.value * 100 < newValue && percentage != 100)
      return null;
  ...
  const handlePreviewVideo = () => {
    ...
    const oldModuleIndex = isFirstClass ? moduleIndex - 1 : moduleIndex;
  ...
  const handleNextVideo = (next) => {
    ...
    if (lock !== 'ok' && next != 'endNext')
      return notification.warn({ message: lock });

    console.log('handleNextVideo(endNext)');
    handleMouseMove();
    // setState({ ...state, playing: true });
  ...
  const onEndVideo = async () => {
    ...
    if (
      ...
      (modules[moduleId] && modules[moduleId].includes(moduleId))
    )
      return null;
    ...
    dispatch({
      type: 'PROGRESS_DONE',
      payload: `${cursoId}//${moduleId}//${classId}`,
    }); //PROGRESS_DISPATCH
  ...
  return (
    <div style={{ width: '100%' }}>
      {moduleId === 'certificado' ? (
        <>
          <PlayerWrapper>
            <CertificadoView student={modules} curso={curso} />
          </PlayerWrapper>
          <BottomControls
            autoplay={autoplay}
