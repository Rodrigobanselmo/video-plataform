...
export function MembersTable({ isLoading, data, filter = true, isClient }) {
  ...
  const authenticating =
    'Usuário já criou sua conta na plataforma, mas não finalizado o cadastro.';
                      ...
                      <TableBComponent>
                        <StatusView status={user?.status}>
                          ...
                          {user?.status === 'Pendente' ? (
                            <>
                              <BootstrapTooltip
                                title={`Revogar acesso a este usuário, o qual você receberá de volta os cursos que foram disponibilizados a ele.`}
                                styletooltip={{ transform: 'translateY(10px)' }}
                              >
                                <section>
                                  <IconLoadButton
