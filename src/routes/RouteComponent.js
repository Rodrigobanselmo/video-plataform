...
export default function RouteComponent({
  ...
  function isAdminRoute() {
    //verifica se rota é admin e se for e user nao tiver access admin ele volta pra app e se for admin vai pra adminRoute

    const ROUTE_HAS_ADMIN =
      rest?.location?.pathname &&
      rest.location.pathname.split('/')[2] &&
      rest.location.pathname.split('/')[2] == 'admin';
    ...
    if (ROUTE_HAS_ADMIN) {
      ...
      if (USER_IS_ADMIN && ROUTE_IS_DASHBOARD) {
        //?se nao tiver admin na rota redireciona pra home do admin
  ...
  function isToInputData() {
    // if (currentUser.manager) return true;
    const HAS_COMPANY =
      currentUser?.permission &&
      Array.isArray(currentUser.permission) &&
      currentUser.permission.includes('co') &&
      !currentUser.permission.includes('coea'); //company
    const HAS_PROFESSION =
      currentUser?.permission &&
      Array.isArray(currentUser.permission) &&
      currentUser.permission.includes('pr'); //professional

    const PERSONAL_DATA =
      currentUser?.name && currentUser?.cpf && currentUser?.cell;
    const COMPANY_DATA =
      !currentUser?.company?.juridica ||
      (currentUser?.company?.juridica && currentUser?.company?.cpfOrCnpj);
    const PROFESSION_DATA = currentUser?.resume && currentUser?.signatureURL;

    const NEED_DATA =
      !PERSONAL_DATA ||
      (HAS_COMPANY && !COMPANY_DATA) ||
      (HAS_PROFESSION && !PROFESSION_DATA) ||
  ...
  function isVerification() {
    ...
    if (!IS_EMAIL_VERIFY && !ROUTE_IS_VERIFY_EMAIL) {
      //verification screen if not verified
      locationRedirect = VERIFY_EMAIL;
      return false;
    } else if (IS_EMAIL_VERIFY && ROUTE_IS_VERIFY_EMAIL) {
      // in verification screen and verify is true go to dashboard
      locationRedirect = DASHBOARD;
      return false;
    } else if (!IS_EMAIL_VERIFY && ROUTE_IS_VERIFY_EMAIL) {
      // preciso verficar email e estou na rota certa
      return true;
    }
