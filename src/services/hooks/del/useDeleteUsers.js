...
async function onDeleteReduce(userToDelete, user, batch) {
  ...
  const reduce = await reduceRef
    .where('id', '==', companyId)
    .where('reduceType', '==', reduceType)
    .get();
  let dataToUpdate = [];

  reduce.forEach((doc) => {
    const isContentSameUser = doc
      .data()
      .data.some((i) => i.uid == userToDelete.uid);
...
async function onAddUser(userToDelete, user, batch) {
  ...
  if (
    userToDelete?.statement &&
    userToDelete.statement[0] &&
    'value' in userToDelete.statement[0]
  ) {
    const products = [
      {
        type: 'deletedUser',
    ...
    newUser.credit =
      Number(newUser.credit) + Number(userToDelete.statement[0].value);
  }
...
export async function deleteUsers(userToDelete, user) {
  //data = array of users {}
...
export function useDeleteUsers() {
  ...
  return useMutation(async (data) => deleteUsers(data, currentUser), {
    //data = array of users {}
    onSuccess: (data, userToDelete) => {
      ...
      if (userToDelete.isPrimaryAccount && userToDelete.access == 'client')
        queryClient.setQueryData('clients', (oldData) => [
          ...oldData.filter((i) => i.uid !== data.uid),
        ]);
      else
